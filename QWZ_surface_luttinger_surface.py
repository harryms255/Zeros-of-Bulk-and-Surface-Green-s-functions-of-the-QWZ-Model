...
# def QHZ_real_space(kx,Ny,t,mu,alpha,symmetry_breaking=0):
...
#     y=0
#     H[2*y,2*y]+=symmetry_breaking
#     H[2*y+1,2*y+1]+=symmetry_breaking
#     return H

# def surface_Greens_function(omega,kx,Ny,t,mu,alpha,SB=0):
#     G=np.linalg.inv((omega+0.0000001j)*np.identity(2*Ny)-QHZ_real_space(kx, Ny, t, mu, alpha,symmetry_breaking=SB))
...
#     GF=surface_Greens_function(omega, kx,Ny, t, mu, alpha,SB)
...
mu=1
SB=0.25
kx_values=np.linspace(-0.5*np.pi,0.5*np.pi,51)
